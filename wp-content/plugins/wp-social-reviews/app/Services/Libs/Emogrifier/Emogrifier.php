...
class Emogrifier
{
    ...
    protected function process(\DOMDocument $xmlDocument)
    {
        ...
        $this->clearAllCaches();
        $this->purgeVisitedNodes();
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_set_error_handler -- Third-party library (Emogrifier), error handler needed for XPath warnings
    ...
    public function handleXpathQueryWarnings( // @codingStandardsIgnoreLine
        ...
        if ($selector !== '') {
            throw new \InvalidArgumentException(
                sprintf('%1$s in selector >> %2$s << in %3$s on line %4$u', esc_html($message), esc_html($selector), esc_html($file), esc_attr($line)),
                1509279985
            );
        }
        ...
        if (isset($context['xPath'])) {
            throw new \RuntimeException(
                sprintf('%1$s in %2$s on line %3$u', esc_html($message), esc_html($file), esc_attr($line)),
                1509280067
            );
        }
