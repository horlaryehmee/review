	...
	public function render_bulk_reviews_page() {
		...
		<div class="wrap">
			...
			<table class="form-table" role="presentation">
				...
				<h2>Reviews</h2>
				<p id="review9ja_reviews_placeholder" class="description">Select a listing to start adding reviews.</p>
				<div id="review9ja_reviews_container"></div>

				<p>
					<button type="button" class="button" id="review9ja_add_review" disabled>Add another review</button>
				</p>

				<?php submit_button( 'Save Reviews', 'primary', 'review9ja_save_reviews', true, [ 'id' => 'review9ja_save_reviews', 'disabled' => 'disabled' ] ); ?>
			</form>
		</div>
		...
		<script>
			(function() {
				...
				const reviewsPlaceholder = document.getElementById('review9ja_reviews_placeholder');
				const addReviewBtn = document.getElementById('review9ja_add_review');
				const submitBtn = document.getElementById('review9ja_save_reviews');
				...
				function setReviewControlsEnabled(enabled) {
					addReviewBtn.disabled = !enabled;
					if (submitBtn) {
						submitBtn.disabled = !enabled;
					}
					if (reviewsPlaceholder) {
						reviewsPlaceholder.style.display = enabled ? 'none' : 'block';
					}
				}
					...
					wrapper.innerHTML = `
						...
						<div class="review9ja-row-grid">
							...
							<div>
								...
								<input type="number" min="1" max="${currentMaxRating}" step="0.5" id="review9ja_rating_${index}" name="reviews[${index}][rating]" class="small-text">
							</div>
							<div>
				...
				function clampRatingValue(input) {
					if (!input) {
						return;
					}
				...
				function wireReviewRow(row) {
					const authorInput = row.querySelector('input[name$="[author]"]');
					const emailInput = row.querySelector('input[name$="[email]"]');
					const ratingInput = row.querySelector('input[name$="[rating]"]');
					if (!authorInput || !emailInput) {
						return;
					}

					const updateEmail = () => {
						const autoEmail = buildEmailFromName(authorInput.value);
						if (!autoEmail) {
							if (emailInput.dataset.auto === '1') {
								emailInput.value = '';
								emailInput.dataset.autoValue = '';
							}
							return;
						}

						const isAuto = emailInput.dataset.auto === '1';
						if (emailInput.value.trim() === '' || isAuto) {
							emailInput.value = autoEmail;
							emailInput.dataset.auto = '1';
							emailInput.dataset.autoValue = autoEmail;
						}
					};

					authorInput.addEventListener('input', updateEmail);
					if (ratingInput) {
						ratingInput.addEventListener('blur', () => clampRatingValue(ratingInput));
						ratingInput.addEventListener('change', () => clampRatingValue(ratingInput));
					}
					emailInput.addEventListener('input', () => {
						const current = emailInput.value.trim();
						const autoValue = emailInput.dataset.autoValue || '';
						if (current === '' || current === autoValue) {
							emailInput.dataset.auto = '1';
						} else {
							emailInput.dataset.auto = '0';
						}
					});

					updateEmail();
					if (ratingInput) {
						clampRatingValue(ratingInput);
					}
				}

				function addReviewRow() {
					if (!listingIdField.value) {
						return;
					}
				...
				function selectListing(id, text) {
					const previousId = listingIdField.value;
					...
					if (!previousId || previousId !== String(id)) {
						resetReviewRows();
					} else if (!reviewsContainer.children.length) {
						addReviewRow();
					}
				...
				function updateRatingFields(listingId) {
					...
					.then(data => {
						...
						ratingLabels.forEach(label => {
							label.textContent = `Rating (1-${maxRating}, .5 allowed)`;
						});
				...
				// Require listing selection before adding reviews
				setReviewControlsEnabled(false);

				const params = new URLSearchParams(window.location.search);
				const preselectId = params.get('listing_id');
				if (preselectId) {
					const preselectTitle = params.get('listing_title');
					const label = preselectTitle ? preselectTitle : `Listing ${preselectId}`;
					selectListing(preselectId, label);
				}
			})();
		</script>
		<?php
	...
	public function save_bulk_reviews() {
		...
		foreach ( $raw_reviews as $index => $review ) {
			...
			if ( $rating_input > 0 && $reviews_class_available ) {
				$rating_input = round( $rating_input * 2 ) / 2;
				$rating_input = max( 1, min( 5, $rating_input ) );
				$stored_rating = $max_rating === 10 ? ( $rating_input * 2 ) : $rating_input;
