...
class InventoryValidation
{
    ...
    public function usedGlobalInventories()
    {
        ...
        foreach ($globalInventoryField as $fieldName => $field) {
            if ($this->isSingleInventoryField($field)) {
                $attachedGlobalInventorySlug = Arr::get($globalInventoryField,$fieldName . '.settings.global_inventory');
            } else {
                ...
                foreach ($options as $option) {
                    ...
                    $attachedGlobalInventorySlug[Arr::get($option, $key)] = Arr::get($option, 'global_inventory');
                }
            }
            $inventoryFields[$fieldName] = $attachedGlobalInventorySlug;
        }
        $this->globalInventoryFieldsName = $inventoryFields;
        $inventoryFieldsFlatten = [];
        array_walk_recursive($inventoryFields, function ($item, $key) use (&$inventoryFieldsFlatten){
            $inventoryFieldsFlatten[$key] = $item;
        });
        $usedInventoryFields = array_unique(array_values($inventoryFieldsFlatten));
        return self::getSubmittedGlobalInventories($usedInventoryFields);
    }
    ...
    public function getGlobalInventoryInfo($slug)
    {
        ...
        $quantity = Arr::get($this->globalInventoryItems, $slug . '.quantity');
        $usedQuantity = Arr::get($this->usedGlobalItemsRecords, $slug, 0);
