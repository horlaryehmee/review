    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        foreach ($keys as $key) {
            if ($key['required'] && empty($feedData[$key['key']])) {
                do_action('fluentform/integration_action_result', $feed, 'failed', __("Skipped insert Zoho CRM  $list_id data for missing required value. Field: ", 'fluentformpro') . $key['key'] . '.');
                return false;
            }
        ...
        if ($response['fields']) {
            ...
            foreach ($response['fields'] as $field) {
                if ($this->isMainField($field)) {
                    ...
                    $other_supported_fields = ['text', 'textarea', 'integer', 'website', 'phone', 'double', 'currency'];
