...
class Simple_Products {
	...
	public function __construct() {
		// Enabled via Theme Options (new) with legacy ACF fallback.
		$enabled = null;
		if ( class_exists( '\MyListing\\Src\\Theme_Options\\Settings_Repository' ) ) {
			$enabled = \MyListing\Src\Theme_Options\Settings_Repository::instance()->get( 'product_vendors_enable', null );
		}
		if ( null === $enabled ) {
			// Legacy ACF option fallback (boolean-like int 0/1).
			$enabled = (bool) get_option( 'options_product_vendors_enable' );
		}
		if ( ! $enabled ) {
			return;
		}
	...
	public function add_product() {
		...
		if ( ! empty( $_FILES['product_featured_image'] ) && ! empty( $_FILES['product_featured_image']['tmp_name'] ) ) {
			$featured_image = c27()->upload_file( $_FILES['product_featured_image'], [ 'image/jpeg', 'image/png', 'image/webp' ] );
		...
		if ( ! empty( $_FILES['product_gallery_images'] ) && ! empty( $_FILES['product_gallery_images']['tmp_name'] ) ) {
			...
			if ( ! empty( $gallery_file_arr ) ) {
				...
				foreach ($gallery_file_arr as $gallery_file) {
					$gallery_image = c27()->upload_file( $gallery_file, [ 'image/jpeg', 'image/png', 'image/webp' ] );
		...
		$product_data = [
			...
			'meta_data' => [],
			// New fields handled by Product_Controller directly
			'stock_status' => $_stock_status__valid ? sanitize_text_field( $_POST['_stock_status'] ) : 'outofstock',
			'date_on_sale_from' => isset( $_POST['_sale_price_dates_from'] ) ? sanitize_text_field( $_POST['_sale_price_dates_from'] ) : '',
			'date_on_sale_to'   => isset( $_POST['_sale_price_dates_to'] ) ? sanitize_text_field( $_POST['_sale_price_dates_to'] ) : '',
			// Optional fields
			'sku'    => isset($_POST['_sku']) ? sanitize_text_field( $_POST['_sku'] ) : '',
			'weight' => isset($_POST['_weight']) ? sanitize_text_field( $_POST['_weight'] ) : '',
			'length' => isset($_POST['_length']) ? sanitize_text_field( $_POST['_length'] ) : '',
			'width'  => isset($_POST['_width']) ? sanitize_text_field( $_POST['_width'] ) : '',
			'height' => isset($_POST['_height']) ? sanitize_text_field( $_POST['_height'] ) : '',
		];
