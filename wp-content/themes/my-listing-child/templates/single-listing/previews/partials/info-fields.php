...
<ul class="lf-contact no-list-style">
    <?php foreach ( (array) $options['info_fields'] as $info ) {
        ...
        if ( ! empty( $content ) ) {
            $icon = isset( $info['icon'] ) ? (string) $info['icon'] : '';

            if ( $icon === '' && isset( $info['label'] ) && is_string( $info['label'] ) ) {
                if ( preg_match( '/\[\[(?:location|address|region|place|map)[^\]]*\]\]/i', $info['label'] ) ) {
                    $icon = 'mi location_on';
                }
            }
            ?>
            <li <?php echo join( ' ', $attributes ) ?>>
            	<?php if ( ! empty( $icon ) ): ?>
                	<i class="<?php echo esc_attr( $icon ) ?> sm-icon"></i>
            	<?php endif ?>
                <?php echo $content ?>
            </li>
        <?php }
